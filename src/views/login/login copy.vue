...
<template>
	<div class="loginContent">
		...
		<div class="r">
			<div class="tab">
				<a-button :type="loginType === 'admin' ? 'primary' : 'default'" @click="setLoginType('admin')"> 管理员登录 </a-button>
				<a-button :type="loginType === 'employ' ? 'primary' : 'default'" @click="setLoginType('employ')"> 员工登录 </a-button>
				<div class="or">
					<p>
						<span>OR</span>
					</p>
			...
			<div class="cont">
				<a-form :model="formState" :label-col="{ style: { width: '50px' } }">
					<a-form-item name="account" :validateStatus="validateStatus === 'account' ? 'error' : ''">
						<a-input v-model:value="formState.account" :placeholder="loginType === 'admin' ? '请输入用户名' : '请输入完整邮箱账户'">
							<template #prefix>
